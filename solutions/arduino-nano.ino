...
void loop() {
  int val = analogRead(7);                    // 0–1023
  int idx = map(val, 0, 1023, 0, 87);          // 88 notes total
  
  if (idx >= 0 && idx < 88 && NOTE_FREQS[idx] != prev_note) {
    Serial.print(NOTE_NAMES[idx]);
    Serial.print(" → ");
    Serial.println(NOTE_FREQS[idx]);
  ...
  if (digitalRead(2) == 1){
   ...
   if (digitalRead(3) == 1){
    ...
    for(int i = 0; i< max_num_notes; i++){
      ...
      delay(500);
      Serial.println(NOTES_FOR_SONG[i]);
      if (NOTES_FOR_SONG[i] == 0){
        break;
        }
      }
    }
